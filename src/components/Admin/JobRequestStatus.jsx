...
const JobRequestStatus = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:8080/application/getAll");
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    try {
      const response = await axios.put(`http://localhost:8080/api/job-requests/${id}`, { status: newStatus });
  ...
  const handleDownload = (filename) => {
    ...
    link.href = `http://localhost:8080/resumes/${filename}`; // Replace with your backend URL
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8080/api/job-requests/${id}`);
