  ...
  return (
    <>
      <div className="text-center my-10">
        <p className="lg:text-3xl pt-10 text-2xl bg-gradient-to-r from-[#00ceff] to-[#0072ff] inline-block text-transparent bg-clip-text">
          ...
          {items.map((item, index) => (
            ...
            >
              <div className="flex  justify-center items-center w-20 h-20 rounded-full  border-4 border-transparent hover:border-blue-500 transition-all duration-300">
                <img className="p-1" src={item.icon} alt={item.label} />
              </div>
              <span className="mt-2 py-4 text-lg font-medium">
                {item.label}
              </span>
            </div>
          ))}
